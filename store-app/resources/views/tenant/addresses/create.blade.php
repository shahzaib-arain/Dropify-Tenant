...
<div class="container mt-4">
    <div class="card shadow-sm">
        <div class="card-header bg-primary text-white">
            <h2 class="h5 mb-0">Add Address for {{ $customer->name }}</h2>
        </div>
        <div class="card-body">
            <form action="{{ route('tenant.addresses.store', ['subdomain' => tenant('subdomain'), 'customer' => $customer->id]) }}" method="POST">
                @csrf
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('line1') is-invalid @enderror" 
                           id="line1" name="line1" value="{{ old('line1') }}" required>
                    @error('line1')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
